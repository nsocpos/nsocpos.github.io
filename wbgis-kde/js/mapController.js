/**
 * mapController.js - Kontrol Peta untuk Eksperimen
 */
...
const MapController = (function() {
    ...
    function showMarkers(data) {
        ...
        filteredData.forEach(item => {
            ...
            let radius = 4;
            if (isHotspot) { fillColor = '#c62828'; radius = 7; }
            else if (isColdspot) { fillColor = '#1565c0'; radius = 7; }
            ...
            marker.bindPopup(`
                <div style="min-width:180px;padding:4px;">
                    <div style="font-weight:600;color:#1a237e;font-size:13px;">${item['NAMA KANTOR'] || 'Fasilitas'}</div>
    ...
    function showHotspots(data) {
        ...
        filteredData.forEach(item => {
            ...
            const marker = L.circleMarker([lat, lng], {
                radius: 8, fillColor: '#c62828', color: '#ff1744',
            ...
            marker.bindTooltip(`
                <div style="text-align:center;">
                    ...
                    <span style="font-size:13px;font-weight:600;">Kepadatan: ${density} titik</span>
                </div>
            `, { permanent: false, direction: 'top', className: 'custom-tooltip hotspot-tooltip' });
            
            marker.bindPopup(`
                <div style="min-width:180px;padding:4px;">
                    <div style="color:#c62828;font-weight:700;font-size:15px;">🔥 HOTSPOT</div>
                    <hr style="margin:4px 0;">
                    <div style="font-weight:600;font-size:13px;">${item['NAMA KANTOR'] || 'Fasilitas'}</div>
                    <table style="font-size:11px;margin-top:4px;width:100%;">
                        <tr><td><b>Regional:</b></td><td>${item['REGIONAL'] || '-'}</td></tr>
                        <tr><td><b>Provinsi:</b></td><td>${item['PROVINSI'] || '-'}</td></tr>
    ...
    function showColdspots(data) {
        ...
        filteredData.forEach(item => {
            ...
            const marker = L.circleMarker([lat, lng], {
                radius: 8, fillColor: '#1565c0', color: '#448aff',
                weight: 2, opacity: 1, fillOpacity: 0.8
            });
            ...
            marker.bindPopup(`
                <div style="min-width:180px;padding:4px;">
                    <div style="color:#1565c0;font-weight:700;font-size:15px;">❄️ COLDSPOT</div>
                    <hr style="margin:4px 0;">
                    <div style="font-weight:600;font-size:13px;">${item['NAMA KANTOR'] || 'Fasilitas'}</div>
                    <table style="font-size:11px;margin-top:4px;width:100%;">
                        <tr><td><b>Regional:</b></td><td>${item['REGIONAL'] || '-'}</td></tr>
    ...
    function fitToData(data) {
        ...
        if (bounds.length > 0) {
            try { map.fitBounds(bounds, { padding: [40, 40], maxZoom: 12 }); }
